...
test("pending Above Pinyin reserves ruby geometry without a visible skeleton", () => {
  assert.doesNotMatch(mainCss, /\.above-reading-pending \.above-reading-plain-cluster::before/u);
});
...
test("full-line base text remains one flex wrapping item", () => {
  const baseFlow = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .lyric-base-flow");
...
test("plain syllable text reserves the same base row as real ruby", () => {
  const cluster = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .furigana-cluster");
  ...
  assert.match(mainCss, /\.furigana-plain-cluster\s*\{\s*justify-items:\s*start/u);
  assert.doesNotMatch(mainCss, /\.word\.furigana-row-reserved\s*\{[^}]*padding-top/u);

  const wrapGroup = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .lyric-wrap-group");
...
test("lyric sidecars remain stable geometry rather than animated blocks", () => {
  ...
  assert.doesNotMatch(animatorSource, /RomajiElement(?:\?\.|\.)style\.(?:scale|transform)/u);
...
test("derived lyric roles keep explicit weight and readable resting paint", () => {
  assert.doesNotMatch(mainCss, /LyricsContent:not\(:has\(\.LyricsNotice\)\)\s*\*\s*\{/u);

  const furigana = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .furigana-reading");
  assert.match(furigana, /font-weight:\s*700/u);
  assert.match(furigana, /--furigana-bright-alpha:\s*var\(--gradient-alpha,\s*0\.85\)/u);
...
test("clipped reading-row words and whole-line sidecars protect block-end ink without changing flow", () => {
  const readingRowHost = ruleBody(
    "#SpicyLyricsPage .LyricsContainer .LyricsContent .has-reading-row"
  );
  assert.match(readingRowHost, /overflow:\s*visible/u);
  assert.match(
    mainCss,
    /\.line\.has-reading-row,\s*#SpicyLyricsPage \.LyricsContainer \.LyricsContent \.line:has\(\.has-reading-row\)/u
  );
  const readingWordRule = mainCss.match(
    /#SpicyLyricsPage \.LyricsContainer \.LyricsContent \.word\.has-reading-row,\s*#SpicyLyricsPage \.LyricsContainer \.LyricsContent \.letterGroup\.has-reading-row\s*\{([\s\S]*?)\}/u
  );
  assert.ok(
    readingWordRule,
    "ordinary and emphasized reading-row hosts must share the clipped-ink guard"
  );
  const readingWord = readingWordRule[1];
  assert.match(readingWord, /--lyric-clip-block-end-guard:\s*0\.125em/u);
  assert.match(readingWord, /padding-block-end:\s*var\(--lyric-clip-block-end-guard\)/u);
  assert.match(
    readingWord,
...
test("every derived row follows the line's unfocused blur once", () => {
  ...
  assert.match(mixedCss, /\.line\.NotSung[\s\S]*?text-shadow:\s*0 0 var\(--BlurAmount,\s*0\)/u);
  assert.match(mixedCss, /\.line\.Sung[\s\S]*?text-shadow:\s*0 0 var\(--BlurAmount,\s*0\)/u);
  assert.match(mixedCss, /\.line\.Active\s*\{[\s\S]*?--BlurAmount:\s*0px\s*!important/u);
...
test("synced furigana and romaji glow only with their active lyric owner", () => {
  ...
  assert.match(animatorSource, /applyWordGlowState\(word,\s*currentGlow\)/u);
});
...
test("timed romaji and line sidecars follow paint-only extra gradient state", () => {
  ...
  assert.match(animatorSource, /line\.HasExtraSidecars[\s\S]*?"--extra-gradient-position"/u);
  assert.doesNotMatch(
    animatorSource,
...
test("Latin reading sidecars stay LTR inside RTL source lines", () => {
  const romanized = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .romanized-below");
  assert.match(romanized, /direction:\s*ltr/u);
  assert.match(romanized, /unicode-bidi:\s*isolate/u);
  assert.match(mainCss, /\.romanized-syllable\s*\{[\s\S]*?--RomanizedGradientDegrees:\s*90deg/u);
  assert.match(
    mainCss,
...
test("syllable furigana follows its timing owner without a masked rectangular glow", () => {
  ...
  assert.match(mainCss, /-webkit-mask-image:\s*linear-gradient/u);
  assert.match(mainCss, /content:\s*attr\(data-furigana\)/u);
  assert.match(
    mainCss,
  ...
  assert.match(animatorSource, /"--timed-furigana-gradient-position"/u);
  const furigana = ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .furigana-reading");
  assert.match(furigana, /background:\s*none\s*!important/u);
  assert.doesNotMatch(mainCss, /\.furigana-reading\s*\{[^}]*background-clip:\s*text/u);
  assert.match(
    ruleBody("#SpicyLyricsPage .LyricsContainer .LyricsContent .timed-furigana-reading"),
...
test("line furigana follows Active line paint without delaying glow", () => {
  ...
  const lineFurigana = ruleBody(
    '.SpicyLyricsScrollContainer[data-lyrics-type="Line"] .furigana-reading'
  );
  ...
  const notSungLineFurigana = ruleBody(".line.NotSung\n  .furigana-reading");
  assert.match(notSungLineFurigana, /color:\s*var\(--furigana-fill-dim\)/u);
  assert.match(notSungLineFurigana, /-webkit-text-fill-color:\s*var\(--furigana-fill-dim\)/u);
  assert.match(
    mainCss,
  ...
  assert.doesNotMatch(mainCss, /\.line:is\(\.Active,\s*\.Sung\)\s+\.furigana-reading/u);
  assert.doesNotMatch(
    mainCss,
  ...
  assert.doesNotMatch(animatorSource, /lineFuriganaFillProgress|--line-furigana-fill-progress/u);
