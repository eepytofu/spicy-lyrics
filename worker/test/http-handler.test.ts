...
const trackQuery =
  "?request_version=16&title=Song&artist_name=First&artist_name=Second&album=Album&duration=240";
...
describe("Worker HTTP boundary", () => {
  ...
  it("rejects malformed track IDs and incomplete metadata", async () => {
    ...
    expect((await handler(request("qq", "?request_version=16&title=Song&duration=240"))).status).toBe(400);
  });

  it("rejects stale request contracts and oversized metadata", async () => {
    const handler = createWorkerHandler();
    expect((await handler(request("qq", trackQuery.replace("request_version=16", "request_version=15")))).status).toBe(426);
  ...
  it("serializes native JSON with the cache and CORS contract", async () => {
    ...
    expect(response.headers.get("Cache-Tag")).toBe("spicy-lyrics-v16");
    expect(response.headers.get("Access-Control-Allow-Origin")).toBe("*");
